...
import { 
  ...
  MapPin, Phone, Mail, Calendar, BatteryCharging, Briefcase, Flag, GraduationCap
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleShare = () => {
    ...
    scripts.forEach(script => {
        if (script.type === 'module' || script.type === 'importmap' || script.src?.includes('react')) {
            script.remove();
        }
    });
    ...
    const filename = `Resume_${data.name.en.replace(/\s+/g, '_')}.html`;
    ...
    if (multiline) {
        return (
            <textarea 
                ...
                className={`w-full border-b border-cyan-400 bg-transparent outline-none focus:border-tech-purple transition-colors ${className}`}
            />
        );
    }
    return (
        <input 
            ...
            value={value} 
            onChange={(e) => handleTextChange(path, e.target.value)}
            className={`w-auto min-w-[50px] border-b border-cyan-400 bg-transparent outline-none focus:border-tech-purple transition-colors ${className}`}
        />
    );
  };

  return (
    <div className="min-h-screen relative pb-20 text-slate-800">
      <Background />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center">
               <div className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-600 to-teal-600">
                  {t(data.name)}
               </div>
              ...
              {!isReadOnly && (
                <>
                  {isEditing ? (
                    <button 
                        onClick={handleSave}
                        className="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-full text-sm font-medium transition-all shadow-lg shadow-cyan-600/30"
                    >
                        <Save size={16} />
                  ...
                  >
                   ...
                   <button 
                    onClick={() => addProject()}
                    className="flex items-center gap-2 px-3 py-2 md:px-4 bg-teal-600 text-white hover:bg-teal-700 rounded-full text-xs md:text-sm font-medium transition-all whitespace-nowrap"
                  >
                    <Plus size={16} />
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 space-y-16">
        ...
        <section className="grid md:grid-cols-2 gap-10 items-center">
            <div className="order-2 md:order-1 space-y-6">
                <div className="text-sm font-semibold text-cyan-600 tracking-wider uppercase">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 text-sm text-slate-600">
                   ...
                   {data.contact.degree && (
                       <div className="flex items-center gap-2">
                           ...
                           <EditableText path="contact.degree" value={t(data.contact.degree)} />
                       </div>
                   )}
                    {data.contact.politicalStatus && (
                        <div className="flex items-center gap-2">
                            <Flag size={16} className="text-cyan-600" />
                            <EditableText path="contact.politicalStatus" value={t(data.contact.politicalStatus)} />
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="order-1 md:order-2 flex justify-center md:justify-end relative">
                <div className="relative w-64 h-64 md:w-80 md:h-80">
                    {/* Decorative Circles */}
                    <div className="absolute inset-0 rounded-full border border-cyan-200/50 animate-[spin_10s_linear_infinite]"></div>
                    <div className="absolute -inset-4 rounded-full border border-teal-100/50 border-dashed animate-[spin_15s_linear_infinite_reverse]"></div>
                    ...
                    <div className="absolute inset-0 rounded-full overflow-hidden border-4 border-white shadow-2xl group bg-slate-100">
                         ...
                         />
                         {isEditing && (
                             <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer z-10" onClick={() => fileInputRef.current?.click()}>
                    ...
                    <div className="absolute bottom-0 right-0 bg-white p-3 rounded-2xl shadow-lg flex items-center gap-2 animate-bounce" style={{ animationDuration: '3s' }}>
                        <div className="bg-cyan-100 p-1.5 rounded-full">
                            <BatteryCharging size={20} className="text-cyan-600" />
                        </div>
                        <div>
        ...
        <section>
             ...
             <div className="flex flex-wrap gap-3">
                 {data.skills.map((skill, idx) => (
                     <div key={idx} className="px-4 py-2 bg-white border border-cyan-100 shadow-sm rounded-xl text-slate-700 text-sm font-medium hover:border-cyan-400 hover:text-cyan-600 transition-colors cursor-default">
                         {t(skill)}
                     </div>
                 ))}
             </div>
        ...
        <section>
             ...
             <div className="grid md:grid-cols-2 gap-6">
                 {data.education.map(edu => (
                     <div key={edu.id} className="bg-white p-6 rounded-2xl border border-cyan-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                         ...
                         <div className="relative z-10">
                             <div className="text-sm text-cyan-600 font-semibold mb-1">{edu.period}</div>
        ...
        <section>
             ...
             <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-cyan-200 before:to-transparent">
                 {data.workExperience.map((work, idx) => (
                     <div key={work.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                         <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-white bg-slate-200 group-hover:bg-cyan-500 transition-colors shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 absolute left-0 md:left-1/2 -translate-x-1/2 z-10">
                             <div className="w-2.5 h-2.5 bg-white rounded-full"></div>
                         </div>
                         <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-6 rounded-2xl border border-cyan-100 shadow-sm hover:shadow-xl transition-all duration-300 ml-16 md:ml-0 relative">
                             {isEditing && (
                                 <button 
                             ...
                             <ul className="space-y-3">
                                 {work.responsibilities.map((resp, rIdx) => (
                                     <li key={rIdx} className="text-sm text-slate-600 flex items-start gap-3 group/li">
                                         <span className="mt-1.5 w-1.5 h-1.5 bg-slate-300 group-hover/li:bg-teal-400 rounded-full flex-shrink-0 transition-colors"></span>
                                         {isEditing ? (
                                             <textarea 
                                                 value={t(resp)} 
                                                 onChange={(e) => updateResponsibility(work.id, rIdx, e.target.value, 'workExperience')}
                                                 className="w-full bg-slate-50 border-b border-slate-200 p-1 text-sm focus:outline-none focus:border-cyan-500"
                                                 rows={3}
                                             />
                                         ) : (
                                 ...
                                 {isEditing && (
                                     <button 
                                        ...
                                        className="text-xs text-cyan-600 hover:underline flex items-center gap-1 mt-2"
                                     >
            ...
            <section>
                ...
                <div className="grid grid-cols-1 gap-6">
                    {data.projects.map((project, idx) => (
                        <div key={project.id} className="bg-white rounded-2xl border border-cyan-100 shadow-sm p-6 hover:shadow-md transition-shadow relative">
                            {isEditing && (
                                <button 
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-2">
                                ...
                                <div className="text-sm font-bold text-teal-600 bg-teal-50 px-3 py-1 rounded-full self-start md:self-auto whitespace-nowrap">
                                    ...
                                    <li key={rIdx} className="text-sm text-slate-600 flex items-start gap-3">
                                        ...
                                        {isEditing ? (
                                            <textarea 
                                                ...
                                                className="w-full bg-slate-50 border-b border-slate-200 p-1 text-sm focus:outline-none focus:border-teal-500"
                                                rows={2}
                                            />
                                        ) : (
                                            <span className="leading-relaxed">{t(resp)}</span>
